...
export function InlineSessionForm({ initialData, onCancel, onSave }: InlineSessionFormProps) {
    // In a real app, we'd use state here to manage the inputs
    // For this mock, we'll just rely on defaultValue to show the concept

    return (
        <Card className="mb-4 border-gray-200 shadow-md animate-in fade-in slide-in-from-top-2 duration-200">
            <CardHeader className="pb-3 border-b border-gray-100 flex flex-row items-center justify-between">
                <CardTitle className="text-sm font-medium text-gray-900">{initialData ? "Edit Session" : "New Session"}</CardTitle>
            ...
            <CardContent className="grid gap-4 py-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                        ...
                        <Input id="title" defaultValue={initialData?.title} placeholder="What did you work on?" className="h-9" autoFocus />
                    </div>
                    <div className="space-y-1.5">
                        ...
                        <Select defaultValue={initialData?.category}>
                            ...
                            <SelectContent>
                                <SelectItem value="CCS Cases">CCS Cases</SelectItem>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1.5">
                            ...
                            <Input id="start" type="time" defaultValue={initialData?.startTime} className="h-9" />
                        </div>
                        <div className="space-y-1.5">
                            <Label htmlFor="end" className="text-xs font-medium text-gray-500">End</Label>
                            <Input id="end" type="time" defaultValue={initialData?.endTime} className="h-9" />
                        </div>
                    </div>

                    <div className="space-y-1.5">
                        <Label className="text-xs font-medium text-gray-500 block mb-1.5">Focus Score</Label>
                        <RadioGroup defaultValue={initialData?.focusScore || "neutral"} className="flex items-center gap-2">
                ...
                <div className="space-y-1.5">
                    ...
                    <Textarea id="notes" defaultValue={initialData?.notes} placeholder="Notes..." className="min-h-[60px] resize-none" />
                </div>
            </CardContent>
